  ...
  <div class="bg-gray-100 min-h-screen">
    <div class="max-w-7xl mx-auto p-4">
      <div v-if="item" class="bg-white rounded-lg shadow-lg overflow-hidden">
        ...
        <div class="md:flex">
          <!-- Image Section -->
          <div v-if="item.image" class="md:w-1/2 relative aspect-w-16 aspect-h-9 md:aspect-none md:h-[400px]">
            <img
              :src="item.image"
              :alt="item.item_name"
          ...
          <div class="md:w-1/2 p-6 space-y-6">
            <div v-if="!isEditMode">
              ...
              <div class="mt-4">
                ...
                <div class="mt-4">
                  <h3 class="text-lg font-semibold mb-2">HSN Code</h3>
                  <p class="text-gray-600">{{ item.gst_hsn_code || 'No HSN code available.' }}</p>
                </div>
...
const fetchItem = createResource({
  ...
  params: {
    ...
    fields: ['name', 'item_code', 'item_name', 'item_group', 'stock_uom', 'description', 'image', 'gst_hsn_code'] // added gst_hsn_code
  },
  onSuccess(data) {
    item.value = data
...
async function saveChanges() {
  ...
  updateItem.submit({
    ...
    fieldname: {
      ...
      image: imageUrl,
      gst_hsn_code: editedItem.value.gst_hsn_code
    }
  })
}
